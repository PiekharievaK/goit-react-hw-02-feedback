    ...
    return (
        <ul className={s.list}>
            {Object.entries(options).map((option) => {
                ...
                return (<li key={objKey} className={s.grades }>{objKey}: <span  className={ s[objKey] }>{value}</span></li>)
    })}
        <li className={s.totalField}>Total feedback: {totalValue}</li>
        <li className={s.totalField}>Positive feedback: {percentageValue ? percentageValue : 0} %</li>
        </ul>
    )
}
